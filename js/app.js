(() => {
    ...
    function write_timer(elements, time) {
        ...
        for (let element of elements) if (element.getElementsByClassName("seconds").length && element.getElementsByClassName("minutes").length) {
            element.getElementsByClassName("seconds")[0].innerHTML = seconds;
            element.getElementsByClassName("minutes")[0].innerHTML = minutes;
            if (element.getElementsByClassName("hours").length) element.getElementsByClassName("hours")[0].innerHTML = hours;
    ...
    function start_timer(className, minutes = 0) {
        ...
        if (minutes) {
            time = minutes * 60;
            if (get_cookie("time")) time = parseInt(get_cookie("time"), 10); else set_cookie("time", time, 1);
        } else time = Math.floor((new Date((new Date).getFullYear(), (new Date).getMonth(), (new Date).getDate() + 1) - new Date) / 1e3);
